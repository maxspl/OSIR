...
class HandlerManager:
    ...
    def __init__(self, db_osir):
        """
            Initializes the HandlerManager with a database connection.
        """
        self.db = db_osir
    ...
    def list(
        ...
        try:
            ...
            for row in rows:
                ...
                row_dict["case_name"] = row[-1]  # Include case_name from join
    ...
    def _row_to_dict(self, row) -> Dict:
        """Standardizes database rows into dictionary format."""
        return {
    ...
    def append_task_ids(self, handler_id: str, new_task_ids: List[str]) -> bool:
        ...
        """
        try:
            result = self.db.execute_query("SELECT task_id FROM osir_handlers WHERE handler_id = %s", (handler_id,), fetch="fetchone")
            if result is None:
